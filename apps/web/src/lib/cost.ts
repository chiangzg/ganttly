 ...
 * Formula: `personDays = Σ (load/100 × capacity × effortDays)` over a task's
 * assignments. `effortDays` contains normal project-calendar working days plus
 * task-local, explicitly marked overtime dates. Merely spanning a weekend or
 * holiday does not imply overtime.
 *
 ...
 * effective effort-day count). Returns 0 for a task with no assignments.
 */
export function computeTaskPersonDays(
  task: Task,
  resources: ReadonlyArray<Resource>,
  cal: ResolvedCalendar,
): number {
  if (task.assignments.length === 0) return 0;
  const effortDays = effectiveTaskDays(task, cal).length;
  const resourceMap = new Map(resources.map((r) => [r.id, r]));
  let total = 0;
  for (const a of task.assignments) {
    const resource = resourceMap.get(a.resourceId);
    const capacity = resource?.capacity ?? 1;
    total += (a.load / 100) * capacity * effortDays;
  }
 ...
 * assignment's `load/100 × capacity × effort-day count`, or 0 if the resource
 * is not assigned. Used by the resource view's drill-down, where each task
 * lane is scoped to a single resource (so the task-wide
 * `computeTaskPersonDays` would over-count by including other assignees).
 */
export function computeAssignmentPersonDays(
  ...
  const capacity = resource?.capacity ?? 1;
  const effortDays = effectiveTaskDays(task, cal).length;
  const pd = (assignment.load / 100) * capacity * effortDays;
...
export function totalPersonDays(
  ...
  for (const t of tasks) {
    ...
    total += computeTaskPersonDays(t, resources, cal);
  }
  return Math.round(total * 100) / 100;
}
