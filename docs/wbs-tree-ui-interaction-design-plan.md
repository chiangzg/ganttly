...
- **编辑模型**:原实现是三个常驻 `<input>`(每敲一键一条 undo 记录),
  现改为静态文本 + 进入编辑态后输入,单命令提交修复 undo 污染。
- **双击语义**:资源没有详情抽屉,双击 = 下钻开合;任务 lane 保留
  双击 = 打开任务抽屉(与任务视图一致)。
  **修订(2026-09-07)**:角色/容量两格例外——双击单元格 = 行内编辑该字段
  (`stopPropagation` 阻断行下钻)。这两个字段没有 F2 之外的任何入口
  (F2/右键「重命名」都只落 name),v0.8.0 落地时遗漏了鼠标入口,用户
  感知为「无法编辑角色/容量」,故补此直达入口;双击姓名区域仍 = 下钻。
