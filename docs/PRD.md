...
- WBS 编号自动生成(如 `1.2.3`),跟随层级变化,编号列扁平对齐(树形箭头/缩进参考线在任务名列);
- 增删任务:Enter 新建同级任务,Tab/Shift+Tab 升降级,Delete 删除;
- 鼠标拖拽行左侧把手:重新排序 + 改层级(行主体不可拖,避免误拖);
- 行内编辑:F2 重命名,Tab 遍历工期/进度;双击行 = 打开详情抽屉;
- 折叠/展开子任务(折叠时父任务带子任务计数徽标;搜索栏提供全部展开/收起)。
...
- 拖拽行首把手 = 排序 + 改层级(行主体不可拖);
- Tab/Shift+Tab = 升降级;
- Alt+Up/Down = 上下移动;
- Enter = 新建同级任务;
- F2 = 重命名;
- Delete = 删除(带二次确认);
- 双击行(任意位置) = 打开编辑抽屉;行内编辑经 F2 进入、Tab 遍历(名称/工期/进度),右键菜单提供「重命名」。
